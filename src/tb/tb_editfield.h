...
public:
	...
	TBTextFragmentContent *CreateFragmentContent(const char *text, int text_len) override;
};
...
public:
	...
	WIDGET_HIT_STATUS GetHitStatus(int x, int y) override;
};
...
public:
	...
	bool GetCustomSkinCondition(const TBSkinCondition::CONDITION_INFO &info) override;
	...
	bool SetText(const char *text) override { return m_style_edit.SetText(text, TB_CARET_POS_BEGINNING); }
	bool GetText(TBStr &text) override { return m_style_edit.GetText(text); }
	...
	void OnMessageReceived(TBMessage *msg) override;
private:
	...
	void Invalidate(const TBRect &rect) override;
	void DrawString(int32 x, int32 y, TBFontFace *font, const TBColor &color, const char *str, int32 len) override;
	void DrawRect(const TBRect &rect, const TBColor &color) override;
	void DrawRectFill(const TBRect &rect, const TBColor &color) override;
	void DrawTextSelectionBg(const TBRect &rect) override;
	void DrawContentSelectionFg(const TBRect &rect) override;
	void DrawCaret(const TBRect &rect) override;
	void Scroll(int32 dx, int32 dy) override;
